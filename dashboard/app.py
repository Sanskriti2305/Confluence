...
.stApp {
    background: 
        radial-gradient(circle at 80% 0%, rgba(223, 255, 65, 0.15), transparent 30%),
        radial-gradient(circle at 90% 70%, rgba(223, 255, 65, 0.08), transparent 30%),
...
.block-container { padding-top: 2rem !important; padding-bottom: 2rem !important; max-width: 1520px !important; }
...
.glass-card { background: rgba(20, 20, 22, 0.5); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 16px; padding: 1.2rem; backdrop-filter: blur(12px); height: 100%; display: flex; flex-direction: column; justify-content: center; }
.card-title { font-family: 'Manrope', sans-serif; font-size: 0.75rem; letter-spacing: 0.2em; color: #A0A0A0; text-transform: uppercase; margin-bottom: 1.2rem; font-weight: 600; }
.metric-val { font-family: 'Cormorant Garamond', serif; font-size: clamp(2rem, 3.5vw, 3.5rem); color: #DFFF41; line-height: 1; margin-bottom: 0.5rem; display: flex; align-items: center; gap: 12px; white-space: nowrap; }
...
.pipeline-container { display: flex; justify-content: space-between; align-items: center; padding: 1rem 0; }
.pipe-step { display: flex; flex-direction: column; align-items: center; gap: 10px; z-index: 2; text-align: center; }
.pipe-circle { width: clamp(36px, 4vw, 48px); height: clamp(36px, 4vw, 48px); border-radius: 50%; border: 1px solid #DFFF41; display: flex; align-items: center; justify-content: center; background: #09090B; }
.pipe-circle.active { background: rgba(223, 255, 65, 0.15); box-shadow: 0 0 20px rgba(223, 255, 65, 0.3); }
.pipe-label { font-size: 0.75rem; font-weight: 600; letter-spacing: 0.1em; color: #EEEEEE; }
.pipe-sub { font-size: 0.7rem; color: #888888; }
.pipe-line { flex-grow: 1; height: 1px; background: rgba(223, 255, 65, 0.5); margin: 0 10px; margin-top: -35px; z-index: 1; }
.case-table { width: 100%; border-collapse: collapse; }
.case-table th { text-align: left; font-size: 0.65rem; color: #888; text-transform: uppercase; letter-spacing: 0.1em; padding-bottom: 12px; border-bottom: 1px solid rgba(255,255,255,0.1); }
.case-table td { padding: 12px 0; font-size: clamp(0.75rem, 1vw, 0.85rem); border-bottom: 1px solid rgba(255,255,255,0.05); white-space: nowrap; }
.case-table tr:hover td { background: rgba(255,255,255,0.02); }
.font-mono { font-family: 'IBM Plex Mono', monospace; }
.text-green { color: #DFFF41; }
.btn-outline { border: 1px solid rgba(223, 255, 65, 0.4); background: transparent; color: #DFFF41; padding: 8px 16px; border-radius: 20px; font-size: 0.7rem; letter-spacing: 0.1em; cursor: pointer; margin-top: 15px; align-self: flex-start; }

/* Streamlit Sidebar overrides for native Radio Buttons */
[data-testid="stSidebar"] { background: rgba(15, 15, 15, 0.45) !important; border-right: 1px solid rgba(255, 255, 255, 0.05) !important; backdrop-filter: blur(20px) !important; width: 240px !important; }
[data-testid="stSidebar"] div[role="radiogroup"] { display: flex !important; flex-direction: column !important; gap: 8px !important; margin-bottom: 2rem !important; }
[data-testid="stSidebar"] div[role="radiogroup"] label { background: transparent !important; border: 1px solid transparent !important; border-radius: 12px !important; padding: 12px 15px !important; margin: 0 !important; cursor: pointer !important; }
...
[data-testid="stSidebar"] div[role="radiogroup"] label p { font-size: 0.85rem !important; color: #888 !important; margin: 0 !important; font-family: 'Manrope', sans-serif !important; }
[data-testid="stSidebar"] div[role="radiogroup"] label:has(input:checked) p { color: white !important; font-weight: 600 !important; }

/* Backend UI overrides */
.case-card { background: rgba(20, 20, 22, 0.5); border: 1px solid rgba(255, 255, 255, 0.08); border-left: 4px solid #e63946; border-radius: 8px; padding: 1rem 1.2rem; margin-bottom: 0.8rem; }
.case-card.high { border-left-color: #e63946; }
.case-card.medium { border-left-color: #f4a261; }
.case-card.low { border-left-color: #2a9d8f; }
...
<svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#DFFF41" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
<path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/>
</svg>
<div style="font-size: 0.7rem; letter-spacing: 0.2em; color: #DFFF41; margin-top: 10px;">CONFLUENCE</div>
</div>
""", unsafe_allow_html=True)
    ...
    st.markdown("""
<div style="position: absolute; bottom: 30px; left: 1rem; right: 1rem; display: flex; align-items: center; gap: 12px;">
<div style="width: 40px; height: 40px; border-radius: 50%; background: #333;"></div>
<div>
<div style="color: white; font-size: 0.9rem;">Sam</div>
<div style="color: #888; font-size: 0.75rem;">Analyst</div>
</div>
<div style="margin-left: auto; color:#DFFF41; font-size:12px;">●</div>
</div>
""", unsafe_allow_html=True)


# ─── PAGE 1: OVERVIEW (HIGH-FIDELITY MOCKUP UI) ──────────────────────────────
if page == "🏠 Overview":
...
<div style="text-align: right; color: #DFFF41; font-size: 0.6rem; letter-spacing: 0.2em; flex-shrink: 0;">
<svg width="100" height="20" viewBox="0 0 100 20">
<circle cx="80" cy="10" r="4" fill="#DFFF41" filter="blur(2px)"/>
<circle cx="80" cy="10" r="2" fill="#FFF"/>
<line x1="0" y1="10" x2="70" y2="10" stroke="#DFFF41" stroke-width="1" opacity="0.3"/>
</svg><br>
...
<svg viewBox="0 0 24 24" fill="none" stroke="#DFFF41" stroke-width="1.5"><path d="M2 12h4l3-9 5 18 3-9h5"/></svg>
{amt_str}
</div>
<div class="metric-label">Flagged Exposure</div>
</div>
""", unsafe_allow_html=True)
...
<div class="glass-card">
<div class="metric-val">
<svg viewBox="0 0 24 24" fill="none" stroke="#DFFF41" stroke-width="1.5"><circle cx="12" cy="5" r="3"/><circle cx="5" cy="19" r="3"/><circle cx="19" cy="19" r="3"/><path d="M12 8v11M7.5 16.5l9-9M16.5 16.5l-9-9"/></svg>
{n_clusters}
</div>
<div class="metric-label">Suspicious Clusters</div>
</div>
""", unsafe_allow_html=True)
    ...
    with m3:
        st.markdown("""
<div class="glass-card">
<div class="metric-val" style="color: #fff;">
<svg viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="1.5"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>
97%
</div>
...
""", unsafe_allow_html=True)
    ...
    with c1:
        st.markdown("""
<div class="glass-card">
<div class="card-title">TOP CASES</div>
<table class="case-table">
<tr><th>Rank</th><th>Cluster ID</th><th>Pattern</th><th>Risk Score</th><th>Amount</th></tr>
<tr><td style="color:#DFFF41; font-weight:bold;">1</td><td>CL_0891</td><td>Circular Flow</td><td class="text-green font-mono">0.94</td><td class="font-mono">$980,540</td></tr>
<tr><td>2</td><td>CL_0732</td><td>Fan-in / Fan-out</td><td class="text-green font-mono">0.89</td><td class="font-mono">$742,210</td></tr>
<tr><td>3</td><td>CL_0611</td><td>Layering</td><td class="text-green font-mono">0.87</td><td class="font-mono">$620,450</td></tr>
<tr><td>4</td><td>CL_0455</td><td>Round Tripping</td><td class="text-green font-mono">0.82</td><td class="font-mono">$512,800</td></tr>
<tr><td>5</td><td>CL_0310</td><td>Structuring</td><td class="text-green font-mono">0.78</td><td class="font-mono">$421,670</td></tr>
</table>
<button class="btn-outline">VIEW ALL CASES &rarr;</button>
</div>
""", unsafe_allow_html=True)

    with c2:
        st.markdown('<div class="glass-card"><div class="card-title">NETWORK CLUSTERS</div>', unsafe_allow_html=True)
        ...
        colors = ['rgba(223, 255, 65, 0.4)'] * len(G.nodes())
        ...
        node_trace = go.Scatter(
            ...
            mode='markers',
            hoverinfo='none',
            marker=dict(showscale=False, color=colors, size=sizes, line_width=0))
...
<span style="color:#DFFF41;">●</span> High Risk Cluster
</div>
</div>
""", unsafe_allow_html=True)

    with c3:
        st.markdown('<div class="glass-card"><div class="card-title">LIQUIDITY RISK HEATMAP</div>', unsafe_allow_html=True)
        df_map = pd.DataFrame({
            'lat': [37.77, 40.71, 51.5, 48.85, 35.68, -23.55, 22.31, 19.07],
            'lon': [-122.41, -74.00, -0.12, 2.35, 139.69, -46.63, 114.16, 72.87],
            'risk': [0.9, 0.8, 0.6, 0.5, 0.95, 0.4, 0.85, 0.7]
        })
        fig_map = go.Figure(go.Scattergeo(
            lon = df_map['lon'], lat = df_map['lat'], mode = 'markers',
            marker = dict(
                size = df_map['risk'] * 20, color = df_map['risk'],
                colorscale = [[0, 'rgba(223,255,65,0.2)'], [1, '#DFFF41']],
                cmin = 0, cmax = 1, line_width=0
            )
        ))
        fig_map.update_layout(
            geo = dict(
                showland = True, landcolor = "rgba(40, 40, 40, 0.5)",
        ...
        st.markdown("""
<div style="display:flex; justify-content: space-between; font-size: 0.6rem; color: #888; text-transform: uppercase; margin-top: 10px;">
<span>Low Risk</span>
<div style="flex-grow: 1; height: 4px; background: linear-gradient(90deg, rgba(255,255,255,0.1), #DFFF41); margin: 5px 15px; border-radius: 2px;"></div>
<span>High Risk</span>
</div>
</div>
""", unsafe_allow_html=True)
...
<div class="pipe-sub">150K+ Txns</div>
</div>
<div class="pipe-line"></div>
<div class="pipe-step">
<div class="pipe-circle">✨</div>
<div class="pipe-label">CLEAN</div>
<div class="pipe-sub">Deduplicate</div>
</div>
<div class="pipe-line"></div>
<div class="pipe-step">
<div class="pipe-circle">🔗</div>
<div class="pipe-label">GRAPH</div>
<div class="pipe-sub">Build Network</div>
</div>
<div class="pipe-line"></div>
<div class="pipe-step">
<div class="pipe-circle">🎯</div>
<div class="pipe-label">SCORE</div>
<div class="pipe-sub">Risk Analysis</div>
</div>
<div class="pipe-line"></div>
<div class="pipe-step">
<div class="pipe-circle active">✓</div>
<div class="pipe-label">RESULT</div>
<div class="pipe-sub">36 Clusters</div>
</div>
</div>
</div>
""", unsafe_allow_html=True)
...
# ─── PAGE 2: LIVE ANALYSIS ───────────────────────────────────────────────────
    ...
    st.markdown(
        "Upload any transaction file and Confluence analyzes it **right now**, live — "
        "this isn't limited to one pre-loaded dataset. Accepts PaySim-style exports "
        "(`nameOrig`, `nameDest`, `amount`, `step`) or a generic format "
        "(`sender_id`, `receiver_id`, `amount`, `timestamp`)."
    )
    st.caption(f"Current compute backend: **{GPU_BACKEND}** · "
               f"Live runs are capped at {LIVE_ROW_CAP:,} rows for demo responsiveness.")
    ...
    if uploaded is not None:
        try:
            ...
            if st.button("▶️ Run Live Analysis", type="primary"):
                ...
                st.caption(f"Backend used for this run: **{backend}**")

                st.markdown("### 🕵️ Suspicious Clusters Found")
                if live_clusters.empty:
                    st.info("No suspicious clusters found in this dataset above the detection threshold.")
                else:
                ...
                if live_liquidity.empty:
                    st.info("Not enough repeated activity per account to score liquidity risk in this dataset.")
                else:
...
Act within: {coord.get('time_to_act_hours','?')}h
</div>
...
""", unsafe_allow_html=True)
        ...
        if not clusters_df.empty:
            filtered = clusters_df[
                (clusters_df["structural_risk_score"] >= risk_threshold) &
                (clusters_df["pattern_type"].isin(pattern_filter))
            ].copy()
            st.dataframe(
                filtered[["cluster_id", "pattern_type", "n_accounts",
                           "structural_risk_score", "total_amount",
                           "time_window_hours"]].sort_values(
...
# ─── PAGE 4: AML CLUSTERS ────────────────────────────────────────────────────
    ...
    if clusters_df.empty:
        ...
        with col_l:
            fig = px.scatter(
                filtered, x="time_window_hours", y="structural_risk_score", size="total_amount",
                color="pattern_type", hover_data=["cluster_id", "n_accounts", "total_amount"],
                title="Risk Score vs Time Compression",
                color_discrete_map={
                    "circular_flow":     "#e63946",
                    "fan_in_fan_out":    "#f4a261",
                    "circular_flow_scc": "#c77dff",
                },
                template="plotly_dark",
            )
            fig.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="rgba(255,255,255,0.8)")
        ...
        with col_r:
            ...
            fig2 = px.pie(
                pattern_counts, names="Pattern", values="Count", title="Pattern Distribution",
                color_discrete_sequence=["#e63946", "#f4a261", "#c77dff"], template="plotly_dark", hole=0.45,
            )
            fig2.update_layout(paper_bgcolor="rgba(0,0,0,0)", font_color="rgba(255,255,255,0.8)")
            st.plotly_chart(fig2, use_container_width=True)

        st.markdown("### Cluster Table")
        st.dataframe(
            filtered[["cluster_id", "pattern_type", "n_accounts", "structural_risk_score", "total_amount", "time_window_hours"]].sort_values(
                "structural_risk_score", ascending=False
...
# ─── PAGE 5: LIQUIDITY RISK ──────────────────────────────────────────────────
    ...
    if liquidity_df.empty:
        ...
        branches_liq = liquidity_df[liquidity_df["entity_type"] == "branch"]
        ...
        with tab1:
            ...
            fig = px.bar(
                top_accts, x="entity_id", y="gap_risk_score", color="gap_risk_score",
                color_continuous_scale=["#2a9d8f", "#f4a261", "#e63946"], title="Top 50 Accounts by Liquidity Gap Risk", template="plotly_dark",
            )
            fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="rgba(255,255,255,0.8)", xaxis={"visible": False})
            st.plotly_chart(fig, use_container_width=True)

            col_l, col_r = st.columns(2)
            with col_l:
                st.metric("Accounts Scored", f"{len(accounts_liq):,}")
            with col_r:
                high_risk = len(accounts_liq[accounts_liq["gap_risk_score"] >= 0.6])
                st.metric("High Risk Accounts", high_risk, delta=f"{high_risk/len(accounts_liq)*100:.1f}% of total", delta_color="inverse")

        with tab2:
            if not branches_liq.empty:
                fig3 = px.bar(
                    branches_liq.sort_values("gap_risk_score", ascending=False), x="entity_id", y="gap_risk_score",
                    color="gap_risk_score", color_continuous_scale=["#2a9d8f", "#f4a261", "#e63946"], title="Branch-Level Liquidity Risk", template="plotly_dark",
                )
                fig3.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="rgba(255,255,255,0.8)")
...
# ─── PAGE 6: BENCHMARK ───────────────────────────────────────────────────────
    ...
    st.markdown("## ⚡ GPU vs CPU Acceleration — Real Measured Results")
    st.markdown("Measured tonight on the same PaySim pipeline, run once on a GPU runtime (NVIDIA T4 + RAPIDS) and once on CPU-only. Comparisons use **throughput** (rows or edges processed per second), not raw wall-clock time.")

    comparison_data = [
        {
            "Stage": "Data Cleaning", "GPU Backend": "cudf.pandas", "GPU Throughput": "302,700 rows/sec",
            "CPU Backend": "pandas", "CPU Throughput": "360,900 rows/sec",
            "Winner": "CPU (this step uses .apply() with custom Python logic, which doesn't vectorize onto GPU)",
        },
        {
            "Stage": "Graph Construction", "GPU Backend": "cuGraph", "GPU Throughput": "1,821,420 edges/sec",
            "CPU Backend": "NetworkX", "CPU Throughput": "74,592 edges/sec",
            "Winner": "🚀 GPU — ~24.4x faster",
        },
        {
            "Stage": "Liquidity Scoring (heuristic)", "GPU Backend": "cudf.pandas", "GPU Throughput": "22,912 accts/sec",
            "CPU Backend": "pandas", "CPU Throughput": "29,606 accts/sec",
            "Winner": "CPU (small per-account groupby aggregation — overhead of GPU data transfer outweighs the work here)",
        },
    ]
    st.dataframe(pd.DataFrame(comparison_data), use_container_width=True, hide_index=True)

    st.markdown("---")
    st.markdown("### 🎯 Detection Accuracy — Ground Truth Verified")
    acc_col1, acc_col2, acc_col3 = st.columns(3)
    acc_col1.metric("Planted Mule Rings", "6")
    acc_col2.metric("Detected", "6")
    acc_col3.metric("Recall", "100%")
    st.caption("Both GPU and CPU runs correctly identified all 6 synthetically planted laundering rings with zero false negatives.")

    if benchmarks:
        st.markdown("---")
        st.markdown("### 📁 Current Session's Raw Benchmark Data")
        st.json(benchmarks)
